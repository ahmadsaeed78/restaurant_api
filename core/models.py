...
class DeliveryOrder(models.Model):
    """
    Represents a multi-item delivery order (e.g., a cart order for delivery).
    """
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, 
        on_delete=models.CASCADE, 
    ...
    def __str__(self):
        return f"DeliveryOrder {self.id} - {self.customer_name or 'Guest'}"

class DeliveryOrderItem(models.Model):
    """
    Represents individual items within a DeliveryOrder.
    """
    order = models.ForeignKey(
        DeliveryOrder, 
        related_name="items", 
    ...
    def __str__(self):
        return f"{self.quantity} x {self.menu_item.name} (DeliveryOrder {self.order.id})"